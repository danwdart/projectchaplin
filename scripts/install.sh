...
cp servers/apache/projectchaplin.conf /etc/apache2/sites-available/projectchaplin.conf &&
...
npm install &&
npm run build &&
chown -R www-data:www-data src/php/public/uploads &&
chown -R www-data:www-data src/php/config/ &&
